...
console.log(`Found ${episodes.length} episodes to generate OG images for:`);
episodes.forEach(ep => console.log(`  - ${ep.slug}: ${ep.guest}`));
...
async function generateOGImage(episode) {
  ...
  // Use first sentence of title as subtitle
  const firstSentence = episode.title.split(/[.!?]/)[0].trim();
...
async function generateMissingImages() {
  console.log(`\nGenerating OG images for ${episodes.length} episodes...\n`);
  ...
  for (let i = 0; i < episodes.length; i++) {
    ...
    console.log(`Processing ${i + 1}/${episodes.length}: ${episode.slug}`);
